...
def print_banner():
    ...
    console.print("[dim]Author: Marc Rivero (@seifreed) | API: https://euvd.enisa.europa.eu/apidoc[/dim]")
    console.print()
...
class EUVDCLIApp:
    ...
    def show_vulnerability_stats(self):
        ...
        table.add_row("Critical Vulnerabilities", str(stats.get('critical_count', 'N/A')))
        table.add_row("Exploited Vulnerabilities", str(stats.get('exploited_count', 'N/A')))
    ...
    def advanced_search_interactive(self):
        ...
        if vendor: kwargs['vendor'] = vendor
        if product: kwargs['product'] = product
        if assigner: kwargs['assigner'] = assigner
        if from_score: kwargs['from_score'] = float(from_score)
        if to_score: kwargs['to_score'] = float(to_score)
        if from_epss: kwargs['from_epss'] = float(from_epss)
        if to_epss: kwargs['to_epss'] = float(to_epss)
        if exploited is not None: kwargs['exploited'] = exploited
        kwargs['size'] = size
        
        console.print(f"[yellow]Searching with filters: {kwargs}[/yellow]")
        data = self.client.search_vulnerabilities(**kwargs)
        
        console.print(f"[green]Found {data.total} total vulnerabilities, showing {len(data.items)} results[/green]")
        pretty_print_json(data)
        return data
...
def search(text, vendor, product, assigner, from_score, to_score, from_epss, to_epss, exploited, size):
    ...
    try:
        ...
        if product: kwargs['product'] = product
        if assigner: kwargs['assigner'] = assigner
        if from_score is not None: kwargs['from_score'] = from_score
        if to_score is not None: kwargs['to_score'] = to_score
        if from_epss is not None: kwargs['from_epss'] = from_epss
        if to_epss is not None: kwargs['to_epss'] = to_epss
        if exploited is not None: kwargs['exploited'] = exploited
        kwargs['size'] = size
